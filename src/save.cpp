...
//  Serial.print("   ");
  ...
  if (ang > BCZ_ANGLE_LEFT) {
    // ???
    // Serial.println("hehe");
    ang = 360.0 - (pow(1.0 - closeness, 0.40) * (360.0 - ang));
    sspeed4 = 0.1 + 0.9 * pow(1.0 - closeness, 0.3); //used to be 2.0
  }
  else if (ang < BCZ_ANGLE_RIGHT) {
    // ???
    // Serial.println("hehe2");
    ang = pow(1.0 - closeness, 0.40) * ang;
    sspeed4 = 0.1 + 0.9 * pow(1.0 - closeness, 0.3); //used to be 2.0
  }
...
void upd_coords() {
    ...
    // if (ub_x < lb_x) ub_x = lb_x; // left sensor is reliable right is not
...
void upd_conf(double angle, double sspeed) {
    ...
    y_conf = pow(1.0 - fabs(lb_y - ub_y) / FIELD_Y, 4.0);
...
//   if (angle < PI) { //ball in front
//     magnitude = getirfront(9);
//   }
//   else magnitude = getirback(9); //ball behind 

//   real_magnitude = //make graph; 

//   double x_dist = real_magnitude * cos(angle); 
//   double y_dist = real_magnitude * sin(angle);
...
//   int values[7];
//   double xspeed = speed * sin(dtor(angle));
//   double yspeed = speed * cos(dtor(angle));
//   double x_sum = 0;
//   double y_sum = 0;


//   for (int i = 0; i < 7; i++){
//     values[i] = analogRead(temt_pin[i]); //do array
//   }

  
//   for (int i = 0; i < 7; i++){
//     double closeness = (values[i] - GREEN_VALUE) / DIFFERENCEGW; //from 0 to 1???? 
//     x_sum += closeness * sin(dtor(temt_angle[i]));
//     y_sum += closeness * cos(dtor(temt_angle[i])); 
//   }

//   double overall_closeness = sqrt(x_sum * x_sum + y_sum * y_sum);
//   double boundary_angle = atan2(x_sum, y_sum);

//   xspeed -= x_sum; 
//   yspeed -= y_sum;
//   temt_final_speed = sqrt(xspeed * xspeed + yspeed * yspeed);
//   temt_final_angle = atan2(xspeed, yspeed) / PI * 180;
